...
- [Guión 03](./guiones/guion_03.zip){target="_blank"} 

- [Repaso Quarto](./guiones/repaso_quarto.zip){target="_blank"} 
...
url <- paste0("https://github.com/perezp44/intro-ds-25-26-web/raw/main/guiones/", file_to_download)
...
- [¿Cómo sublir los trabajos individuales?](./mas-cosas/entrega_trabajo_individual.html){target="_blank"} 
...
```{bash}
#| eval: false
auditpol /set /category:"Seguimiento detallado" /success:enable
```


```{bash}
#| eval: false
Get-WinEvent -LogName Security -MaxEvents 5000 | Where-Object {$_.Id -eq 4688 -and $_.TimeCreated -gt (Get-Date).AddHours(-2.2)} | ForEach-Object {
    $xml = [xml]$_.ToXml()
    $process = $xml.Event.EventData.Data | Where-Object {$_.Name -eq 'NewProcessName'} | Select-Object -ExpandProperty '#text'
    [PSCustomObject]@{
        Hora = $_.TimeCreated
        Programa = Split-Path $process -Leaf
        RutaCompleta = $process
    }
} | Where-Object {
    $_.RutaCompleta -notlike "*\Windows\System32\*" -and
    $_.RutaCompleta -notlike "*\Windows\SysWOW64\*" -and
    $_.Programa -notlike 'svchost*' -and
    $_.Programa -notlike 'conhost*' -and
    $_.Programa -notlike 'backgroundTask*' -and
    $_.Programa -notlike 'RuntimeBroker*' -and
    $_.Programa -notlike 'dllhost*' -and
    $_.Programa -notlike 'taskhostw*' -and
    $_.Programa -notlike 'SearchIndexer*' -and
    $_.Programa -notlike 'WmiPrvSE*' -and
    $_.Programa -notlike 'audiodg*' -and
    $_.Programa -notlike 'dwm*' -and
    $_.Programa -notlike 'csrss*' -and
    $_.Programa -notlike 'lsass*' -and
    $_.Programa -notlike 'services*' -and
    $_.Programa -notlike 'winlogon*' -and
    $_.Programa -notlike 'fontdrvhost*' -and
    $_.Programa -notlike 'smss*'
} | Group-Object Programa | ForEach-Object {$_.Group | Sort-Object Hora -Descending | Select-Object -First 1} | Sort-Object Hora -Descending | Format-Table Hora, Programa -AutoSize
```
...
```{bash}
#| eval: false
log show --predicate 'eventMessage contains "LAUNCHING"' --style compact --last 140m 2>/dev/null | \
grep -iE "\.(app|Application)" | \
grep -vE "com\.apple\.|system|daemon|helper|agent" | \
awk '{print $1, $2, $NF}' | \
sed 's/.*\///' | \
sed 's/\.app.*//' | \
awk '!seen[$3]++' | \
